---
title: Contributing
description: How to contribute to eslint-plugin-ai-guardrails — local setup, architecture, and pull request guidelines.
icon: GitPullRequest
---
...
<Step>
### Detection
Update `src/cli/utils.ts` -> `detectProject()` to identify the framework via dependencies or unique files (e.g., `svelte.config.js`).
</Step>
<Step>
### Templates
Open `src/cli/templates.ts` and add a new framework object to `TPLS`. Include the appropriate `eslintConfigMjs` template and the correct `packageScripts` (for linting and building).
</Step>
<Step>
### Execution
In `src/cli/index.ts`, map the newly detected framework boolean to the template you created in Step 2.
</Step>
</Steps>
