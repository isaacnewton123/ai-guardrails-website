...
description: Prevent "god functions" by limiting the effective line count inside function and method bodies.
icon: Code
---
...
<Callout type="info">
  Arrow functions with expression bodies (e.g., `const fn = () => 42`) are not checked — they are inherently single-expression.
</Callout>
