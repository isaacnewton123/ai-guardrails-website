---
title: no-ai-obvious-comments
description: Detect comments that simply restate the following line of code — a common AI assistant pattern.
...
<Callout type="info">
  The rule only checks **line comments** (`//`), not block comments (`/* */`). It only checks comments immediately followed by a statement on the next line.
</Callout>
...
- Codebases with strict documentation requirements that mandate comments on every statement
- Generated documentation blocks (JSDoc) — this rule only targets inline `//` comments
